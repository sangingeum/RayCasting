...
sf::VertexArray Scene::createSquareVertices(float width, float height) {
...
sf::VertexArray Scene::createCircleVertices(size_t numVertices, float radius) {
...
sf::VertexArray Scene::createBoundaryVertices() {
...
std::shared_ptr<CRender> Scene::addBlock(float width, float height, float translateX, float translateY) {
...
void Scene::addStationaryRayCaster() {
	// Make a raycaster
	auto entity = m_entityManager->addEntity();
...
void Scene::addFollowingRayCaster(size_t numRays, bool drawTriangle, bool drawRay, bool sweep) {
	// Make a raycaster
	auto entity = m_entityManager->addEntity();
	auto circleVA = createCircleVertices(30, 10.f);
	auto cRender = entity->addComponent<CRender>(circleVA);
